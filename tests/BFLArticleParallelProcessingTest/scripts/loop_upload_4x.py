#!/usr/bin/env python3
"""Upload the 12 test articles (6 languages each) 4 times in a row.

Each iteration: bump articleVersion -> upload all 12 files -> clear the
...
def bump_versions(input_dir: Path) -> None:
    base = int(time.time() * 1000)
    for i, name in enumerate(FILES):
        path = input_dir / name
        with path.open("r", encoding="utf-8") as fh:
            data = json.load(fh)
        data["articleVersion"] = base + i
        with path.open("w", encoding="utf-8") as fh:
            json.dump(data, fh, ensure_ascii=False)
    print(f"  Versions bumped (base={base})")
...
def main() -> None:
    ...
    for loop_num in range(1, loops + 1):
        ...
        print("Step 1: bump articleVersion")
        bump_versions(input_dir)
        ...
        result = subprocess.run(
            [
                ...
                "--no-pre-clear",
                "--output-prefix", f"report_loop{loop_num}",
            ],
            check=False,
        )
